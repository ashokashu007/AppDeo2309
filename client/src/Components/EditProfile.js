...
function EditProfile() {
  ...
  useEffect(() => {
    if (
      ((fnInputRef.current.value = storeObj.loginDetails.firstName),
      (lnInputRef.current.value = storeObj.loginDetails.lastName),
      (ageInputRef.current.value = storeObj.loginDetails.age),
      (emailInputRef.current.value = storeObj.loginDetails.email),
      setProfilePic(
        `http://localhost:3456/${storeObj.loginDetails.profilePic}`
      ))
    ) {
    } else {
      // navigate("/");
